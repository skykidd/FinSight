...
class _ScanPageState extends State<ScanPage> {
  ...
  String _extractedText = ""; // Holds the formatted JSON for the dialog viewer
  ...
  bool _isTaxable = false; // Used for Income
  bool _isTaxDeductible = false; // Used for Expenses
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      await _scanWithLocalAPI(_imageFile!);
    } catch (e) {
      setState(() => _isScanning = false);
    }
  }

  // --- 2. FASTAPI MULTIPART CALL NOW USING .ENV ---
  Future<void> _scanWithLocalAPI(File image) async {
    try {
      // PULL URL FROM .ENV FILE (Fallback to local if it's missing for some reason)
      final apiUrl =
          dotenv.env['RECEIPT_SCANNER_API_URL'] ??
          'http://10.0.2.2:8000/extract-receipt';
